...
| **工具调用（v1.3.0）** | 标准 OpenAI `tools`/`tool_choice` 参数：协议指令注入 + 输出解析，还原为标准 `tool_calls` 响应（流式/非流式） |
| **风控自动重试（v1.3.0）** | 识别频率限制/风控错误后**不报错**，静默等待自动重试（次数/间隔可调），期间流式连接心跳保活 |
| **无流式上限（v1.3.0）** | 移除 30 分钟硬上限，长回答不限时；停滞看门狗保留（默认 3 分钟无数据才判定死连接） |
| **云端零残留（v1.3.0）** | 每次对话结束自动删除云端新建的会话，官网对话列表不留 API 痕迹（成功/失败/重试丢弃都清理） |
...
| `tools` | array | OpenAI Function Calling 工具定义（提示词级模拟，见下节） |
| `tool_choice` | string/object | `"auto"`（默认）/ `"required"` / `"none"` / `{"type":"function","function":{"name":"..."}}` |
...
上游 Qwen 没有原生 function calling，本服务采用**提示词协议模拟**：请求带 `tools` 时自动注入协议指令，模型以固定标记行输出调用，服务端实时解析并还原为标准 OpenAI 响应——非流式返回 `message.tool_calls` + `finish_reason:"tool_calls"`；流式返回 `delta.tool_calls` 帧。工具执行结果用 `role:"tool"` 消息回传即可继续对话。
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `QPP_MAX_STREAM_MINUTES` | 不设 = 无上限 | 流式硬上限分钟数（0.05~480）；v1.3.0 起默认移除上限，需限制时再设 |
| `QPP_IDLE_TIMEOUT_MINUTES` | 3 | 流中途无数据的停滞判定（真死连接保护，一般无需调整） |
| `QPP_THROTTLE_RETRIES` | 3 | 风控/限频自动重试次数（设 0 = 关闭重试，立即报错） |
| `QPP_THROTTLE_WAIT_MS` | 20000 | 重试基准等待毫秒，逐轮递增（20s/40s/60s），最小 1000 |
| `QPP_KEEP_CHATS` | 不设 = 用完即删 | 设 `1` 保留云端对话记录（默认自动删除，保持官网列表干净） |
...
**Q: 长回答跑到 10/30 分钟被截断？**
v1.1.7 是固定 10 分钟上限，v1.1.8 放宽到 30 分钟，**v1.3.0 起默认彻底移除上限**——只要模型还在持续输出就不会被本地切断（停滞看门狗仅在连续 3 分钟完全无数据时收尾）。若仍需限制，设环境变量 `QPP_MAX_STREAM_MINUTES`（分钟数）后重启即可；Windows 下：`set QPP_MAX_STREAM_MINUTES=60` 后在同一终端窗口运行 `qspp`（或写入系统环境变量）。在无上限模式下仍被中断且错误提示"风控/停滞"的，属于上游问题而非本地截断。

**Q: tools 工具调用可靠吗？**
上游 Qwen 没有原生 function calling，v1.3.0 采用提示词协议模拟（原理见 API 说明）。简单工具（天气/查询/计算类）实测可靠（qwen3.7-plus 流式/非流式均已验证）；极复杂嵌套 schema 偶发解析失败——失败时原始输出原样保留在正文，不会静默丢失。纯对话（不传 tools）完全不受影响。

**Q: 触发频率限制/风控后会自动恢复吗？**
v1.3.0 起会。识别到风控/429 类错误后：自动删除本次临时会话（云端不残留）→ 静默等待 20s×轮次 → 自动重试（最多 3 轮），期间流式连接通过 SSE 注释行保活不断流，且已有输出时不会重复重试（避免内容重复）。全部轮次耗尽才返回带对策的错误。等待时长/次数可用 `QPP_THROTTLE_WAIT_MS` / `QPP_THROTTLE_RETRIES` 调整；设 `QPP_THROTTLE_RETRIES=0` 恢复旧版"立即报错"行为。
...
**Q: 官网对话列表会被 API 请求撑满吗？**
不会。v1.3.0 起每次对话结束（无论成功/失败/重试丢弃）都会自动删除云端新建的会话，官网列表保持干净（实测已验证）。删除失败仅影响列表整洁，不影响对话结果；确需保留历史可设 `QPP_KEEP_CHATS=1`。
