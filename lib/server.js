...
const { QwenClient, QwenError, FALLBACK_MODELS } = require('./qwen');
const { json, readBody, sseWrite, uuid, nowSec } = require('./util');
...
async function streamChatCompletions(req, res, cfg, body) {
  ...
  for (const m of messages) {
    if (Array.isArray(m.content)) m.content = m.content.filter(p => p && p.type === 'text').map(p => p.text).join('\n');
  ...
  try {
    ...
    const code = e instanceof QwenError ? e.code : 'UPSTREAM_ERROR';
    res.write('data: ' + JSON.stringify({ error: { message: e.message, type: 'api_error', code, status } }) + '\n\n');
    logReq(req.method, req.url, status, Date.now() - t0, '[' + code + ']');
    res.write('data: [DONE]\n\n');
    res.end();
    return;
  }
...
async function handleAdmin(req, res, cfg, pathname, body) {
  ...
  if (req.method === 'POST' && pathname === '/admin/api/test-chat') {
    ...
    try {
      ...
      sseWrite(res, 'error', { message: e.message, code: e.code || 'ERROR' });
    }
    res.end();
    return;
  }
    ...
    try {
      ...
      if (pathname === '/v1/models' && req.method === 'GET') {
        ...
        try {
          ...
          return json(res, 200, openaiModels(FALLBACK_MODELS.map(m => ({ id: m.id, name: m.name }))));
        }
      }
      ...
      if (pathname === '/v1/chat/completions' && req.method === 'POST') {
        ...
        try {
          ...
          const status = e instanceof QwenError ? e.status : 502;
          const code = e instanceof QwenError ? e.code : 'UPSTREAM_ERROR';
          logReq(req.method, req.url, status, Date.now() - t1, '[' + code + ']');
          return json(res, status, { error: { message: e.message, type: status === 401 ? 'authentication_error' : 'api_error', code } });
        }
      ...
      try { json(res, 500, { error: { message: 'internal error: ' + e.message, type: 'api_error' } }); } catch (e2) {}
    }
  };
}
