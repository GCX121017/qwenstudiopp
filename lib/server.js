...
const { buildToolInstructions, renderToolMessages, extractToolCalls, ToolCallStreamFilter } = require('./toolsim');
...
// 流式/非流式共用预处理 (v1.3.0):
//   role 归一化 -> 附件上传(content 数组扁平化) -> tool/assistant.tool_calls 消息渲染
//   -> tools 协议指令注入(以 system 消息插入, buildFlatContent 会放入 [System Instructions])
// 返回 { files, toolsActive }; 失败抛 QwenError (调用方负责 JSON 错误响应)
async function prepareChatRequest(client, body, messages) {
  for (const m of messages) {
    if (!['user', 'assistant', 'system', 'tool'].includes(m.role)) m.role = 'user';
  }
  const files = await resolveAttachments(client, messages);
  renderToolMessages(messages);
  let toolsActive = false;
  if (Array.isArray(body.tools) && body.tools.length && body.tool_choice !== 'none') {
    const instr = buildToolInstructions(body.tools, body.tool_choice);
    if (instr) {
      messages.unshift({ role: 'system', content: instr });
      toolsActive = true;
    }
  }
  return { files, toolsActive };
}
...
async function streamChatCompletions(req, res, cfg, body) {
  ...
  // v1.3.0: 预处理并入 prepareChatRequest (含 tools 指令注入)
  let files = [];
  let toolsActive = false;
  try {
    ({ files, toolsActive } = await prepareChatRequest(client, body, messages));
  } catch (e) {
  ...
  // v1.3.0: tools 模式下正文经流式过滤器剥离 <tool_call> 块(跨 chunk 安全, 坏块回灌正文)
  const streamFilter = toolsActive ? new ToolCallStreamFilter() : null;
  let usage = null;
  let answerLen = 0;
  let finished = 'stop';
  try {
    const result = await client.chatStream({
      ...
      onEvent: (evt) => {
        if (evt.type === 'thinking') {
          ...
          const safe = streamFilter ? streamFilter.push(evt.delta) : evt.delta;
          if (safe) { answerLen += safe.length; send({ content: safe }, null); }
        } else if (evt.type === 'retry_wait') {
          // v1.3.0 风控静默重试: 发 SSE 注释行保活 (OpenAI SDK/主流客户端自动忽略注释行), 客户端不断流不报错
          res.write(': throttled by upstream, retrying in ' + Math.round(evt.waitMs / 1000) + 's (attempt ' + evt.attempt + '/' + evt.maxRetry + ')\n\n');
    ...
    if (streamFilter) {
      ...
      if (tail) { answerLen += tail.length; send({ content: tail }, null); }
      if (streamFilter.toolCalls.length) {
        // v1.3.0: 还原 OpenAI 流式 tool_calls (arguments 一帧整段, openai SDK 按增量聚合兼容)
        streamFilter.toolCalls.forEach((tc, i) => {
          send({ tool_calls: [{ index: i, id: tc.id, type: 'function', function: { name: tc.name, arguments: tc.arguments } }] }, null);
        });
        finished = 'tool_calls';
      }
    } else if (!answerLen && result.answer) {
      // 罕见: 事件没推正文但聚合有 (防御; tools 模式下聚合含标记原文, 不可直接外发)
  ...
  res.write('data: [DONE]\n\n');
  res.end();
  const tcNote = streamFilter && streamFilter.toolCalls.length ? ' +' + streamFilter.toolCalls.length + ' tool_calls' : '';
  logReq(req.method, req.url, 200, Date.now() - t0, answerLen + 'ch' + tcNote);
}
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      if (pathname === '/v1/chat/completions' && req.method === 'POST') {
        ...
        // v1.3.0: tools 模式 -> 解析 <tool_call> 块还原 OpenAI tool_calls / finish_reason
        let content = result.answer;
        let toolCalls = [];
        if (toolsActive) {
          const parsed = extractToolCalls(result.answer);
          content = parsed.cleanText;
          toolCalls = parsed.toolCalls;
        }
        const msg = { role: 'assistant', content: toolCalls.length ? (content || null) : content };
        if (result.thinking) msg.reasoning_content = result.thinking;
        if (toolCalls.length) {
          msg.tool_calls = toolCalls.map(tc => ({ id: tc.id, type: 'function', function: { name: tc.name, arguments: tc.arguments } }));
        }
        logReq(req.method, req.url, 200, Date.now() - t1, result.answer.length + 'ch' + (toolCalls.length ? ' +' + toolCalls.length + ' tool_calls' : ''));
        return json(res, 200, {
