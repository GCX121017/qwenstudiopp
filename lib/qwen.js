...
//  - 高频调用触发 Baxia 滑块风控 -> 客户端节流 + v1.3.0 自动识别/静默等待重试
//  - v1.3.0: 云端对话用完即删 (DELETE /api/v2/chats/{id}, 实测 2026-09 返回 {success:true});
//    失败/重试丢弃的临时对话同样清理, 云端列表不留 API 痕迹
'use strict';
...
// 流式硬上限 (v1.3.0): 默认彻底无上限(0)。停滞看门狗(默认 3 分钟无数据)已足够防真死连接,
// 分钟级硬上限只会误杀超长回答(v1.1.6 固定 10 分钟误杀过, v1.1.8 改 30 分钟仍会误杀长代码生成)。
// 需要上限的用户: 设 QPP_MAX_STREAM_MINUTES=<分钟> (0.05~480); 设 0/不设 = 无上限。
const maxStreamMinutes = () => envMinutes('QPP_MAX_STREAM_MINUTES', 0, 0, 480);
const idleTimeoutMinutes = () => envMinutes('QPP_IDLE_TIMEOUT_MINUTES', 3, 0.05, 60);

// 风控重试参数 (v1.3.0): 识别到频率限制/风控时静默等待后自动重试 (不向客户端报错)
const envInt = (name, def, min, max) => {
  const v = Number(process.env[name]);
  if (!Number.isFinite(v)) return def;
  return Math.min(max, Math.max(min, Math.round(v)));
};
const throttleRetries = () => envInt('QPP_THROTTLE_RETRIES', 3, 0, 10);
const throttleWaitMs = () => envInt('QPP_THROTTLE_WAIT_MS', 20000, 1000, 300000);
const keepChats = () => process.env.QPP_KEEP_CHATS === '1'; // 设 1 保留云端对话(默认用完即删)

// 风控/频率限制错误识别: 覆盖 HTTP 429、流中 upstream_error、UPSTREAM_EMPTY(风控掐断最常见表现)。
// 明确排除业务类错误(鉴权/参数/文件), 避免无意义重试。
const THROTTLE_PAT = /429|rate.?limit|too.?many|throttl|风控|频率|滑块|baxia|busy|overload|try.?again|稍后|频繁/i;
function isThrottleError(e) {
  if (!e) return false;
  const code = String(e.code || '');
  if (code === 'UPSTREAM_429' || code === 'UPSTREAM_EMPTY' || code === 'THROTTLE_RATE_LIMIT') return true;
  if (/^(AUTH_FAILED|NO_TOKEN|BAD_REQUEST|FILE_TOO_LARGE|PARSE_FAILED|TOKEN_INVALID_CHARS|HEADER_INVALID_CHARS|NETWORK_TIMEOUT)$/.test(code)) return false;
  const msg = String(e.message || '');
  if (/HTTP 429/.test(msg)) return true;
  return THROTTLE_PAT.test(code) || THROTTLE_PAT.test(msg);
}
function fmtMs(ms) {
...
class QwenClient {
  ...
  // v1.3.0: role=tool 渲染为 Function 前缀 (配合 toolsim.renderToolMessages 的 [Function result] 文本)
  static buildFlatContent(messages) {
    ...
    for (const m of hist) {
      const role = m.role === 'assistant' ? 'Assistant' : m.role === 'tool' ? 'Function' : 'User';
  ...
  // 删除云端对话 (v1.3.0): DELETE /api/v2/chats/{id}, 官网前端同款 (实测 2026-09 返回 {success:true})
  // 尽力而为: 上游端点若变更仅返回 false 并记日志, 不影响主流程
  async deleteChat(chatId) {
    if (!chatId) return false;
    try {
      const resp = await this.request('/api/v2/chats/' + encodeURIComponent(String(chatId)), { method: 'DELETE', headerTimeoutMs: 15000 });
      if (resp.status === 200 || resp.status === 204) return true;
      if (resp.status === 404) return true; // 已不存在 = 目的已达
      const t = await resp.text().catch(() => '');
      process.stderr.write('[qwen] DELETE /api/v2/chats 返回 HTTP ' + resp.status + ': ' + String(t).slice(0, 120) + '\n');
      return false;
    } catch (e) {
      process.stderr.write('[qwen] 云端对话删除请求失败: ' + ((e && e.message) || e) + '\n');
      return false;
    }
  }

  // 对话用完即删 (v1.3.0): 成功/失败/重试丢弃都清理云端痕迹; fire-and-forget, 失败仅记日志
  _disposeChat(chatId, why) {
    if (!chatId || keepChats()) return; // keepChats()=1 时用户显式要求保留, 跳过删除
    this.deleteChat(chatId).then(ok => {
      if (!ok) process.stderr.write('[qwen] 云端对话清理未确认 (' + why + '): ' + chatId + '\n');
    }).catch(() => {});
  }

  /**
   * 流式对话 (v1.3.0 外层: 风控静默重试 + 对话用完即删)
   * @param {object} p {model, messages, thinking, files?, onEvent?}
   *   onEvent(evt) 回调接收解析后的事件; 新增事件 {type:'retry_wait', attempt, maxRetry, waitMs, message}
   *   (风控等待通知, 客户端可显示状态/发 SSE keep-alive); 返回 {answer, thinking, steps, docs, queries, usage, model}
   */
  async chatStream(p) {
    const maxRetry = throttleRetries();
    const waitMs = throttleWaitMs();
    let contentEmitted = false; // 已向客户端推过模型内容(thinking/content)则不再重试, 避免重复输出
    const userOnEvent = typeof p.onEvent === 'function' ? p.onEvent : null;
    const wrappedOnEvent = (evt) => {
      if (evt && (evt.type === 'content' || evt.type === 'thinking')) contentEmitted = true;
      if (userOnEvent) { try { userOnEvent(evt); } catch (e) { /* 回调异常不中断流 */ } }
    };
    for (let attempt = 0; ; attempt++) {
      let chatId = null;
      try {
        const result = await this._chatStreamOnce(p, wrappedOnEvent, id => { chatId = id; });
        this._disposeChat(chatId, 'done');
        return result;
      } catch (e) {
        this._disposeChat(chatId, 'failed'); // 失败的临时对话同样不留在云端
        const canRetry = attempt < maxRetry && !contentEmitted && isThrottleError(e);
        if (!canRetry) throw e;
        const wait = waitMs * (attempt + 1); // 递增等待: 20s/40s/60s
        process.stderr.write('[qwen] 检测到频率限制/风控 (' + (e.code || 'UNKNOWN') + '), 已清理临时对话, '
          + Math.round(wait / 1000) + 's 后自动重试 (' + (attempt + 1) + '/' + maxRetry + ')\n');
        if (userOnEvent) {
          try { userOnEvent({ type: 'retry_wait', attempt: attempt + 1, maxRetry, waitMs: wait, message: String(e.message || '').slice(0, 200) }); } catch (e2) { /* 忽略 */ }
        }
        await sleep(wait);
      }
    }
  ...
  // 单次尝试 (原 chatStream 主体): 内部新建云端对话并发送; 失败由外层决定是否重试
  async _chatStreamOnce(p, onEvent, setChatId) {
    ...
    if (typeof setChatId === 'function') setChatId(chatId);
    const payload = {
      stream: true,
      version: '2.1',
    ...
    const docList = [];
    let queries = [];
    // 看门狗 (v1.3.0): 停滞检测默认保留; 分钟级硬上限默认关闭(0), 仅显式设置 QPP_MAX_STREAM_MINUTES 时生效
    let hardFired = false, idleFired = false, lastDataAt = Date.now();
    const hardMs = maxStreamMinutes() * 60000;   // 0 = 无上限
    const idleMs = idleTimeoutMinutes() * 60000;
    const hardTimer = hardMs > 0 ? setTimeout(() => { hardFired = true; try { reader.cancel('hard-timeout'); } catch (e) {} }, hardMs) : null;
      ...
      while (true) {
        ...
        for (const evt of events) {
          ...
          if (typeof onEvent === 'function') {
            try { onEvent(evt); } catch (e) { /* 回调异常不中断流 */ }
          }
        }
      }
    } finally {
    ...
    if (hardFired) {
      // v1.1.8: 部分正文 + 到达上限(仅显式设置过上限时) -> 在正文尾部追加可见截断标记
      const note = '\n\n---\n[Qwen Studio++] 已达到 ' + fmtMs(hardMs) + '流式上限, 回复在此截断。'
        + '如需无上限: 设环境变量 QPP_MAX_STREAM_MINUTES=0 (默认) 后重启服务';
...
module.exports = { QwenClient, QwenSseParser, QwenError, FALLBACK_MODELS, QWEN_BASE, describeFetchError, maxStreamMinutes, idleTimeoutMinutes, mimeToKind, FILE_KINDS, isThrottleError, throttleRetries, throttleWaitMs, keepChats };
